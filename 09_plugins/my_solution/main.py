...
from payment_plugin_manager import load_plugins, plugin_names, get_plugin
...
def handle_payment(total: Decimal) -> None:
    payment_type = input(
        f"What payment method would you like to use? {'/'.join(plugin_names())}:"
    )

    if payment_type in plugin_names():
        # Process the payment
        p = get_plugin(payment_type)
